...
5. 自由存储区与堆
    1. 自由存储区泛指有 new/delete 分配与释放的动态内存区域。 所以自由存储区所指并不是单一的区域， 当我们重载 operator new/delete 函数的时候，可以从任意的地方分配内存，堆、全局/静态变量区等。 但是只要是通过 new/delete 表达式分配的内存区域就叫自由存储区。在大部分编译器下， operator new/delete 底层实际是调用 malloc/free 来分配函数的， 这时候自由存储区和堆可以说是等价的。
    
    2. 堆(与数据结构的堆不同)，由 malloc/free 分配释放的动态内存区域。申请大小由用户代码决定。